...
ggplot(sunapee, aes(x = year, y = ice_off_doy))+
  ...
  theme_dark()+
  labs(title = 'Year .vs. Ice off Day of Year', x = 'Time(yrs)', y = 'Ice of Day of Year')
...
fit <- lm(ice_off_doy ~ year, data = ice_data)
fit
...
The direction of the slope is upward. This means that the ice off doy increases as the year increases, which does match my expectations. 
...
R2 - 0.09876. This means they are not strongly related. 
...
```
The histogram shows they are not perfectly normally distributed. Therefore, the residuals only partially fit the linear regression. 
...
The p-value for the slope is 2e-16. Since this value is far smaller than 0.05, we reject the null hypothesis that the slope is equal to zero. Therefore, the slope is statistically significantly different from zero.
...
The slope is −0.098 days per year, meaning the ice-off date is occurring about 0.10 days earlier each year. To observe a one week change, it would take approximately 71 years. While the slope is statistically significant, the rate of change is relatively small per year. However, over several decades, this results in a meaningful ecological shift.
...
I am moderately comfortable using this linear model for identifying long-term trends, but less comfortable using it for precise prediction. The slope is statistically significant (p < 2e-16), indicating strong evidence that the ice-off date is changing over time. However, the R² value is approximately 0.09876, meaning the model explains only about 9.876% of the variation in ice-off dates. Additionally, the residuals show considerable spread, suggesting substantial unexplained variability. Therefore, while the model is useful for detecting overall trends, it is limited in predictive accuracy for individual years.
...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  ...
  labs(title = 'Ice off Day .vs. Year including slope and Intercept', x = 'Time(yr)', y = 'Ice off Day of Year')
...
**Answer 11:**
Predicted ice off day for 2040 = 293.896 - 0.09847(2040) = 93.1

```{r}

quantile(sunapee$resid, probs = c(0.05, 0.95), na.rm = TRUE)

```
lower bound = 93.1 + (-8.95649) = 84.14351
upper bound = 93.1 + (24.17267) = 117.27267
90% confidence level = [84.14, 117.27]
width of interval = 11.27 - 84.14 = 33.13 days
...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  geom_point(alpha = 0.4) +
  geom_abline(
    intercept = coefficients(fit_early)[1],
    slope = coefficients(fit_early)[2],
    color = 'blue',
    size = 1
  )+
  geom_abline(
    intercept = coefficients(fit_late)[1],
    slope = coefficients(fit_late)[2],
    color = 'red',
    size = 1
  )+
  theme_classic()+
  labs(title = 'Ice off Day of Year .vs. Time with slope and intercepts shown based on time', 
       x = 'Time(yr)',
       y = 'Ice off Day of Year')
```
Before 1970, the slope was −0.110 days per year, meaning ice-off was occurring about 0.11 days earlier per year. After 1970, the slope increased in magnitude to −0.302 days per year, indicating ice-off is now occurring about 0.30 days earlier per year. Since the magnitude of the slope is substantially larger in the later period, the ice-off day is changing more quickly in recent years. Both slopes are statistically significant, and the higher R² value in the later period suggests a stronger relationship in recent decades. Therefore, the data provide evidence that the rate of change has accelerated over time.
